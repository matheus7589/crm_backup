...
<div class="panel panel-default panel-vault">
   <div class="panel-heading" role="tab" id="<?php echo 'vaultEntryHeading-'.$entry['id']; ?>">
      ...
      <?php if($entry['creator'] == get_staff_user_id() || is_admin()){if($entry['description'] != 'server_info'){?>
...
<div class="modal fade" id="entryModal" tabindex="-1" role="dialog">
   <div class="modal-dialog" role="document">
      <?php echo form_open(admin_url('clients/vault_entry_create/'.$client->userid),array('data-create-url'=>admin_url('clients/vault_entry_create/'.$client->userid),'data-update-url'=>admin_url('clients/vault_entry_update'), 'id' => "form_vault")); ?>
      <div class="modal-content">
         ...
         <div class="modal-footer">
            ...
            <button type="button" class="btn btn-info" onclick="verify_description();"><?php echo _l('submit'); ?></button>
         </div>
      </div>
      <!-- /.modal-content -->
      <?php echo form_close(); ?>
...
<script>
   ...
   function edit_vault_entry(id){
      //Verifica description por javascript
   ...
   function verify_description()
   {
       ...
       if(x == "server_info"){
           ...
           alert_float('danger', 'Não foi possivel alterar.<br>Escolha outra descrição e salve.');
           return false;
       }
       else {
            document.getElementById('form_vault').submit();
       }
   }
</script>
<?php } ?>
