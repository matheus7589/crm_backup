...
array_push($join, "LEFT JOIN tblclients ON tblclients.userid = tblcontacts.userid");

$where = array();
if(is_partner())
{
    array_push($where, " AND tblclients.partner_id = ".get_staff_partner_id());
}
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, array('tblcontacts.id as id', 'tblcontacts.userid', 'is_primary','(SELECT count(*) FROM tblcontacts c WHERE c.userid=tblcontacts.userid) as total_contacts'));
...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<a href="tel:'.$aRow['tblcontacts.phonenumber'].'">'.$aRow['tblcontacts.phonenumber'].'</a>';
    ...
    $outputActive = '<div class="onoffswitch">
                <input type="checkbox" data-switch-url="'.admin_url().'clients/change_contact_status" name="onoffswitch" class="onoffswitch-checkbox" id="c_'.$aRow['id'].'" data-id="'.$aRow['id'].'"' . ($aRow['tblcontacts.active'] == 1 ? ' checked': '') . '>
                <label class="onoffswitch-label" for="c_'.$aRow['id'].'"></label>
            </div>';
            // For exporting
            $outputActive .= '<span class="hide">' . ($aRow['tblcontacts.active'] == 1 ? _l('is_active_export') : _l('is_not_active_export')) . '</span>';
