...
class Clients extends Admin_controller
{
    ...
    public function activation()
    {
        ...
        $output = array();
        /**
         * Obs: o executável params esta definido nas variaveis de ambiente.
         */
        exec("params teste 1 2 3",$output);

        var_dump($output);

    }

    public function gerar_lum(){
        $this->load->model('activation_model');
        $data['date'] = '09/01/2018';
        $data['userid'] = '264';
        $this->activation_model->gerar_lum($data);
    }
    ...
    public function client($id = '')
    {
        ...
        if(is_partner() && !is_client_partner($id))
            access_denied('customers');
        ...
        if(PAINEL == INORTE) {
            $this->load->model("activation_model");
//            $data['codigo_ativacao'] = $this->activation_model->get_codigo_ativacao(array('userid' => $client->userid));
        }

        $this->load->model("model_certificate_model");

        $data["model_certificates"] = $this->model_certificate_model->all();

        $this->load->model("issuing_certificate_model");

        $data["issuing_certificates"] = $this->issuing_certificate_model->all();
...
//                dd($data);
        ...
        if ($contact_id == '') {
            ...
            $data['clients_relation'] = $this->clients_model->get_clients_relation($contact_id);
        ...
        $data['client'] = $this->clients_model->get_client($customer_id);
        $data['customer_permissions'] = $this->perfex_base->get_contact_permissions();
        $data['title'] = $title;
        $this->load->view('admin/clients/modals/contact', $data);
    }

    public function connect_contact($client_id){

        $data['contacts'] = $this->clients_model->get_contacts();
        $data['client_id'] = $client_id;
        $data['title'] = "Adicionar Contato Existente";
        $this->load->view('admin/clients/modals/connect_contact', $data);
    }

    public function save_connect_contact($client_id, $contact_id){
        if ($client_id && $contact_id){
            $response = $this->clients_model->save_connect_contact($client_id, get_client_contact_by_id($contact_id)->contactid);
            if ($response){
                echo json_encode(array(
                    'message' => 'success',
                ));
    ...
    public function delete_contact($customer_id, $contactid)
    {
        ...
        $this->clients_model->delete_contact($contactid);
        redirect(admin_url('clients/client/' . $customer_id . '?tab=contacts'));
    }

    public function  delete_relation_contact($clientid, $contactid){
        if (!has_permission('customers', '', 'delete')) {
            if (!is_customer_admin($clientid)) {
                access_denied('customers');
            }
        }

        $clientcontact = get_clientcontact_by_relation($clientid, $contactid);
        $this->clients_model->delete_relation_contact($clientcontact);
        redirect(admin_url('clients/client/' . $clientid . '?tab=contacts'));
    }

    public function delete_relation_from_modal_contact($clientid, $contactid){
        if (!has_permission('customers', '', 'delete')) {
            if (!is_customer_admin($clientid)) {
                access_denied('customers');
            }
        ...
        $clientcontact = get_clientcontact_by_relation($clientid, $contactid);
        $del = $this->clients_model->delete_relation_contact($clientcontact);
        if($del){
            echo json_encode(array(
                'message' => 'success',
            ));
    ...
    public function client_relation_table(){
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $sIndexColumn = "id";
            $sTable = 'tblclientscontacts';
            $sJoin = array(
                " join tblclients on tblclients.userid = tblclientscontacts.clientid ",
            );
            $sWhere = array();
//            $contactid = 403;
            array_push($sWhere, " AND tblclientscontacts.contactid = " . $contactid);
            $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $sJoin, $sWhere, array(
                'company',
                '(tblclientscontacts.contactid) as contactid',
            ));
            ...
            foreach ($rResult as $aRow) {
                ...
                for ($i = 0; $i < count($aColumns); $i++) {
                    ...
                    if ($aColumns[$i] == "clientid") {
                        $_data = '<a target="_blank" href="' . admin_url("clients/client/" . $aRow["clientid"] . '/' . $aRow["contactid"]) . '">' . $aRow["company"] . '</a>';
                    }
                    if ($aColumns[$i] == "1")
                        $_data = '<button class="btn btn-danger" onclick="delete_relation(' . $aRow["clientid"] . ', ' . $aRow["contactid"] . '); return false;" style="margin: auto; vertical-align: middle"><i class="fa fa-times"></i></button>';
                    $row[] = $_data;
                }
    ...
    public function get_clientcontact_number($clientid, $contactid){
        if (!has_permission('customers', '', 'delete')) {
            if (!is_customer_admin($clientid)) {
                access_denied('customers');
            }
    ...
    /* Change client status / atualiza / nao atualiza */
    public function change_client_atualiza($id, $status)
    {
        if ($this->input->is_ajax_request()) {
            $this->clients_model->change_client_atualiza($id, $status);
        }
    }
...
//        }
    ...
    public function senha_quantum()
    {
        if($this->input->post())
        {
            $data = $this->input->post();
            if($data["tipo"] == "modulos")
            {
                if(QuantumLiberaModulos::setModules($data))
                    set_alert("success","Módulos atualizado com sucesso!");
                else
                    set_alert("danger", "Erro ao atualizar módulos.");
                header("location: ".admin_url("clients/client/".$data["userid"]."?group=activation_quantum"));
            }
            else
            {
                $senha = QuantumPrivateSenha::index($data);
                if ($data['tipo'] == "ajax-gerasenha" || $data['tipo'] == "ajax-enviasenhasite")
                    echo json_encode($senha);
            }
        }
    }

    public function get_ativa_clients(){

        $clientes = $this->clients_model->get();

        echo '<div class="col-md-12 mbot10">
                <div class="row">
                    <div class="col-md-3">
                        <label for="segmento">Segmento:</label>
                        <select name="segmento" id="segmento" class="form-control selecpicker">
                            <option>Teste</option>
                        </select>
                    </div>
                </div>
        ...
        echo '<div class="col-md-12">
                <table class="table dt-table" id="atualiza">
                    <thead>
                        <th>Id</th>
                        <th>Empresa</th>
                ...
                foreach ($clientes as $cliente){
                    $contato_principal = $this->clients_model->get_contact($cliente['primary_contact']);
                    if($contato_principal){
                        $contato_principal = '<a target="_blank" href="' . admin_url('clients/client/' . $cliente['userid'] . '?contactid=' . $contato_principal->id) . '">' . $contato_principal->firstname . '</a>';
                    }else{
                        $contato_principal = 'Contato principal não Informado';
                    }

                    $versao = '<button onclick="list_versoes(' . $cliente['userid'] . ');" type="button" class="label btn btn-info">Info</button>';

                    $toggleActive = '<div class="onoffswitch" data-toggle="tooltip" data-title="' . _l('Ativar ou Desativar atualização do Cliente') . '">
                        <input type="checkbox" data-switch-url="' . admin_url().'clients/change_client_atualiza" name="checkbox" class="onoffswitch-checkbox" id="atualiza_' . $cliente['userid'] . '" data-id="' . $cliente['userid'] . '" ' . ($cliente['atualiza'] == 1 ? 'checked' : '') . '>
                        <label class="onoffswitch-label" for="atualiza_' . $cliente['userid'] . '"></label>
                    </div>';

                    echo '<tr>
                              <td>' . '#' . $cliente['userid'] . '</td>
                              <td data-order="' . $cliente['userid'] . '"><a target="_blank" href="' . admin_url('clients/client/' . $cliente['userid']) . '">' . $cliente['company'] . '</a></td>
                              <td>' . $contato_principal . '</td>
                              <td>' . $cliente['cnpj_or_cpf'] . '</td>
    ...
    public function get_versoes($userid){

        $this->db->where('userid', $userid);
        $versoes = $this->db->get('tblversoes')->result_array();

        echo '<div class="col-md-12">
                <table class="table dt-table" id="versoes">
                    <thead>
                        <th>Id</th>
                        <th>Executável</th>
                        <th>Versão</th>
        ...
        foreach ($versoes as $versao){
            ...
            echo '<tr>
                              <td>' . '#' . $versao['id'] . '</td>
                              <td data-order="' . $versao['id'] . '">' . $versao['executavel'] . '</td>
                              <td>' . $versao['versao'] . '</td>
                          </tr>';
        }
        echo '</tbody>
