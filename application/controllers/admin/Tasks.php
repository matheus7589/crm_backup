...
class Tasks extends Admin_controller
{
    ...
    public function addgp()
    {
        if($this->input->post())
        {
            ...
            if($data['mastertype'] == 0)
            {
                if ($data['typeex'] == 0)
                    $this->db->query("DELETE FROM tblstafftaskassignees WHERE taskid = 0 AND staffid = " . $data['staffidex']);
                else if ($data['typeex'] == 1)
                    $this->db->query("DELETE FROM tblstafftasksfollowers WHERE taskid = 0 AND staffid = " . $data['staffidex']);
            }
            else if($data['mastertype'] == 1)
            {
                if ($data['type'] == 0)
                    $this->db->query("INSERT INTO tblstafftaskassignees (staffid,taskid,assigned_from) VALUES (" . $data['staffid'] . ",0,0)");
                else if ($data['type'] == 1)
                    $this->db->query("INSERT INTO tblstafftasksfollowers (staffid,taskid) VALUES (" . $data['staffid'] . ",0)");
            }
        }
    }
    ...
    {
        $data['title'] = "Grupos de tarefas";
        $data['task'] = $this->tasks_model->get(0);
        $data['default_assi'] = $this->db->query("SELECT tblstafftaskassignees.*, tblstaff.profile_image FROM tblstafftaskassignees INNER JOIN tblstaff ON tblstaff.staffid = tblstafftaskassignees.staffid WHERE taskid = 0")->result_array();
        $data['default_foll'] = $this->db->query("SELECT tblstafftasksfollowers.*, tblstaff.profile_image FROM tblstafftasksfollowers INNER JOIN tblstaff ON tblstaff.staffid = tblstafftasksfollowers.staffid WHERE `taskid` = 0")->result_array();
//            Seguidores - tblstafftasksfollowers
        $data['staff'] = $this->staff_model->get('', 1);
        $this->load->view('admin/tasks/groups', $data);
    }
    ...
    public function list_tasks($id = '')
    {
        ...
        $data['title'] = _l('tasks');
//        var_dump($this->session->tempdata('task'));
//        var_dump($this->session->tempdata('assigned_task'));
    ...
    public function detailed_overview()
    {
        ...
        $data['members'] = $this->staff_model->getc();
    ...
    public function task($id = '')
    {
        ...
        if ($this->input->get('replyid')) {
            $data['replyid'] = $this->input->get('replyid');
        }
    ...
    public function verify_tasks()
    {
        $tasks = $this->tasks_model->get_user_tasks_current();
        if (count($tasks) > 0) {
            $data['verify'] = 1;
        } else {
            $data['verify'] = 0;
    ...
    {
        if (has_permission('tasks', '', 'edit') || has_permission('tasks', '', 'create')) {
            if($this->tasks_model->verify_assignee($this->input->post('taskid'), array('staffid' => $this->input->post('assignee')))){
                echo json_encode(array(
                    'fail' => 'O colaborador já está assinalado para esta tarefa'
                ));
            }else{
                echo json_encode(array(
    ...
    {
        if (has_permission('tasks', '', 'edit') && has_permission('tasks', '', 'create')) {
            if(!$this->tasks_model->verify_assignee($taskid, array('id' => $id)) && PAINEL == QUANTUM){
                echo json_encode(array(
                    'fail' => 'não foi possível remover',
                    'message' => 'O usuário não se encontra atribuido à tarefa'
                ));
            }else{
                ...
                echo json_encode(array(
                    'success' => $success,
                    'message' => $message,
                    'not_task_modal' => $taskid == 0 ? true : false,
                ));
            }
...
//        $data['staff'] = $this->staff_model->getc('', 1, array("role" => 1), false, true);
    ...
    public function insere()
    {
        ...
        update_option('task_waiting_alert_time',$data['task_waiting_alert_time']);
        update_option('task_waiting_limit_time',$data['task_waiting_limit_time']);
        update_option('task_waiting_alert_sound',isset($data['task_waiting_alert_sound'])?$data['task_waiting_alert_sound']:"0");
        update_option('task_waiting_limit_sound',isset($data['task_waiting_limit_sound'])?$data['task_waiting_limit_sound']:"0");
        update_option('task_waiting_alert_sound_type',$data['task_waiting_alert_sound_type']);
        update_option('task_waiting_limit_sound_type',$data['task_waiting_limit_sound_type']);
        update_option('task_main_color',$data['task_main_color']);
        update_option('painel_task_refresh_time',$data['painel_task_refresh_time']);
        update_option('task_waiting_alert_time_attendance',$data['task_waiting_alert_time_attendance']);
        update_option('task_waiting_limit_time_attendance',$data['task_waiting_limit_time_attendance']);
...
//        logActivity(json_encode($data));
        ...
        redirect(admin_url('tasks/config_painel'));
