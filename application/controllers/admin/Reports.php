...
class Reports extends Admin_controller
{
    ...
    public function fleet_control()
    {
        if ($this->input->is_ajax_request())
            $this->perfex_base->get_table_data('fleet');
        $partnerid = get_staff_partner_id();
        $data['staffs'] = $this->db->select("CONCAT(firstname,' ',lastname) as staffname,staffid")->where("partner_id",$partnerid)->where("active",1)->get("tblstaff")->result_array();
        $data['vehicles'] = $this->db->select("descricao,vehicleid")->get("tblfleetvehicles")->result_array();
        $data['title'] = "Relatório de Saídas";
        $this->load->view("admin/reports/fleet",$data);
    }
    ...
    public function supply()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->input->get('date')) {
                ...
                $dateW .= ' AND data BETWEEN "' . $date_from . '" AND "' . $date_to . '"';
            }
            if ($this->input->get('vehicle')) {
                $dateW .= " AND tblfleetsupply.vehicleid = ".$this->input->get('vehicle');
            }
            if ($this->input->get('posto')) {
                $dateW .= " AND posto = '".$this->input->get('posto')."'";
            }
            if ($this->input->get('type') == "desempenho") {
                echo json_encode($this->reports_model->get_desempenho($dateW));
                die();
            }
            if ($this->input->get('type') == "table") {
                ...
                $aColumns = $select;
                $sIndexColumn = "supplyid";
                $sTable = 'tblfleetsupply';
                $where = array();
                if($dateW != "")
                    array_push($where," AND ".$dateW);

                $result = data_tables_init($aColumns, $sIndexColumn, $sTable, array("INNER JOIN tblfleetvehicles ON tblfleetsupply.vehicleid = tblfleetvehicles.vehicleid"), $where, array());
                ...
                foreach ($rResult as $aRow) {
                    ...
                    for ($i = 0; $i < count($aColumns); $i++) {
                        ...
                        if ($aColumns[$i] == "data")
                            $_data = \Carbon\Carbon::parse($aRow[$aColumns[$i]])->format("d/m/y H:i:s");
                        else if ($aColumns[$i] == "valortotal")
                            $_data = round($aRow[$aColumns[$i]], 2) . " R$";
                        else if ($aColumns[$i] == "precoporlitro")
                            $_data .= " R$";
                        else if ($aColumns[$i] == "litro")
                            $_data .= " L";
        ...
        $data['title'] = "Relatório de Abastecimentos";
        $data['vehicles'] = $this->db->select("descricao,vehicleid")->get("tblfleetvehicles")->result_array();
        $data['postos'] = $this->db->query("SELECT DISTINCT posto FROM `tblfleetsupply`")->result_array();

        /*********************** */
            $data['desempenho'] = $this->reports_model->get_desempenho();

        /*********************** */
        $this->load->view("admin/reports/supply",$data);
    }
...
/*
    ...
    public function equipments()
    {
        if ($this->input->is_ajax_request())
        {
            ...
            $result = data_tables_init($aColumns, $sIndexColumn, $sTable, array(), $where, array('rel_id'));
            $output = $result['output'];
            $rResult = $result['rResult'];
            foreach ($rResult as $aRow) {
                $row = array();
                for ($i = 0; $i < count($aColumns); $i++) {
                    ...
                    if ($aColumns[$i] == "rel_type")
//                        $_data = json_encode(get_relation_values($aRow["rel_id"],$aRow["rel_type"]));
                        $_data = json_encode(get_relation_data($aRow["rel_type"],$aRow["rel_id"])   );
        ...
        $data['title'] = "Relatório da movimentação de Equipamentos";
        $this->load->view("admin/reports/equipments",$data);
    }
}
