...
//        $this->use_submenu    = false;
        ...
        $data['companies'] = get_companies(get_contact_user_id());

        if(count($data['companies']) > 1) {
            $data['title'] = 'Acesso à Empresa';
            $this->view = 'company_list';
        }else{
            ...
            //TODO vai ter q ser por $_SESSION mesmo :(
//            dd($_SESSION);
            if(isset($_SESSION['open_ticket']) && $_SESSION['open_ticket'] == true){
//                $this->view = 'open_ticket';
                $this->view = 'access_screen';
                unset($_SESSION['open_ticket']);
            }else{
                $where['status'] = 1;
    ...
    public function set_user_data($clientid){
...
//            $this->form_validation->set_rules('department', _l('clients_ticket_open_departments'), 'required');
            ...
            if ($this->form_validation->run() !== false) {
                $dataAux = $this->input->post();
                $dataAux['department'] = 1;
                $id = $this->tickets_model->add($dataAux);
                if ($id) {
    ...
    public function player($id,$key = '')
    {
        render_player($id,$key);
    }
}
