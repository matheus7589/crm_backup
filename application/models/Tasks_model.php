...
class Tasks_model extends CRM_Model
{
    ...
    public function get_user_tasks_current()
    {
        $this->db->where('id IN (SELECT taskid FROM tblstafftaskassignees WHERE staffid = ' . get_staff_user_id() . ')');
    ...
    public function verify_assignee($id, $where = array())
    {
        $this->db->where('taskid', $id);
        $this->db->where($where);
        $task = $this->db->get('tblstafftaskassignees')->row();
        if ($task)
            return true;
    ...
    public function get_statuses()
    {
        $statuses = do_action('before_get_task_statuses', array(
            array(
                ...
                'order' => 1,
                'filter_default' => $this->session->tempdata('task') != null && in_array(1, $this->session->tempdata('task')) ? true : false,
            ),
            array(
                'id' => 4,
                'color' => '#03A9F4',
                ...
                'order' => 2,
                'filter_default' => $this->session->tempdata('task') != null && in_array(4, $this->session->tempdata('task')) ? true : false,
            ),
            array(
                'id' => 3,
                'color' => '#2d2d2d',
                ...
                'order' => 3,
                'filter_default' => $this->session->tempdata('task') != null && in_array(3, $this->session->tempdata('task')) ? true : false,
            ),
            array(
                'id' => 2,
                'color' => '#adca65',
                ...
                'order' => 4,
                'filter_default' => $this->session->tempdata('task') != null && in_array(2, $this->session->tempdata('task')) ? true : false,
            ),
            array(
                'id' => 5,
                'color' => '#84c529',
                ...
                'order' => 100,
                'filter_default' => $this->session->tempdata('task') != null && in_array(5, $this->session->tempdata('task')) ? true : false,
            )
        ));
    ...
    public function do_kanban_query($status, $search = '', $page = 1, $count = false, $where = array(), $sort = array())
    {
        $tasks_where = '';
        ...
        $this->db->select('id,name,duedate,startdate,status, addedfrom, priority, dateadded');
        ...
        if ($count == false) {
            if (isset($sort['sort_by']) && $sort['sort_by'] && isset($sort['sort']) && $sort['sort']) {
                if ($sort['sort_by'] == 'priority') {
                    ...
                    if ($sort['sort'] == 'ASC') {
                        usort($result, array($this, "compare_priority_asc"));
                        return $result;
                    }
                    usort($result, array($this, "compare_priority_desc"));
                    return $result;
                }
            }
    ...
    function compare_priority_asc($a, $b)
    {
        return $a['priority'] < $b['priority'];
    }
    ...
    public function add($data, $clientRequest = false)
    {
        ...
        // Linha antiga que setava o status diretamente para "Em progresso"
        ...
        ///////////////////////////////////////////////////////////////////
        // Linha nova que seta o status da tarefa diretamente para "não iniciado"
        $data['status'] = 1;
        /////////////////////////////////////////////////////////////////////////
        ...
        $dataat = $this->db->query("SELECT * FROM tblstafftaskassignees WHERE taskid = 0")->result_array();
        $dataft = $this->db->query("SELECT * FROM tblstafftasksfollowers WHERE taskid = 0")->result_array();

        $this->db->insert('tblstafftasks', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            foreach ($dataat as $dataa) {
                $dataad['staffid'] = $dataa['staffid'];
                $dataad['taskid'] = $insert_id;
                $dataad['assigned_from'] = 0;
                $this->db->insert('tblstafftaskassignees', $dataad);
            }
            foreach ($dataft as $dataf) {
                $datafd['staffid'] = $dataf['staffid'];
                $datafd['taskid'] = $insert_id;
                $this->db->insert('tblstafftasksfollowers', $datafd);
            }
...
//            $this->_send_customer_contacts_notification($data['taskid'], 'task-commented-to-contacts');
    ...
    public function get_task_assignees_by_ticketid_and_status($id, $status)
    {
        $this->db->select('tblstafftaskassignees.id as id,tblstafftaskassignees.staffid as assigneeid,assigned_from,firstname,lastname,is_assigned_from_contact, tblstafftasks.id as taskid, tblstafftasks.name as nome');
        $this->db->from('tblstafftaskassignees');
        $this->db->join('tblstaff', 'tblstaff.staffid = tblstafftaskassignees.staffid', 'left');
        $this->db->join('tblstafftasks', 'tblstafftasks.id = taskid', 'left');
        $this->db->where('rel_type', 'ticket');
        $this->db->where('rel_id', $id);
        $this->db->where('tblstafftasks.status', $status);
        return $this->db->get()->result_array();
    }

    public function get_tasks_by_ticketid($ticketid){
        $this->db->where('rel_type', 'ticket');
    ...
    public function update_ticket_status_after_task_complete($task_relid, $status)
    {
        $this->db
            ->where('ticketid', $task_relid)
            ->update('tbltickets', array(
                'status' => $status,
            ));
    }

    public function check_number_of_tasks($taskid, $status)
    {
        $key_statuses = array(
            1 => 26, // Nao iniciado // DEV - Espera
            2 => 34, // Aguardando feedback // DEV - Reanalise
            3 => 24, // Em teste // DEV - Teste
            4 => 33, // Em progresso // DEV - Atendimento
            5 => 32, // Concluido // DEV - Atendido
        );

        $aux = 0;
        $this->db->select('status, rel_id');
        $this->db->where('id', $taskid);
        $_task = $this->db->get('tblstafftasks')->row();

        $count_tasks = $this->db
        ->select('id, status')
            ->where('rel_id', $_task->rel_id)
            ->where('rel_type', 'ticket')
            ->get('tblstafftasks')->result_array();

        $ticket = $this->db
            ->select('status')
            ->where('ticketid', $_task->rel_id)
            ->get('tbltickets')->result_array();

        if (count($count_tasks) > 1) {
            foreach ($count_tasks as $row) { // Conta a quantidade de tarefas
                if ($row['status'] == $status) { //status 4 em progresso 3 - em espera
                    //$stats = 33; // dev - atendimento
                    //$stats = 24; // dev - espera
                    $aux++;
                }
            }
            if(($aux == count($count_tasks)) && $count_tasks[0]['status'] == 5){
                /** Atualiza o status do ticket caso todas as tarefas estejam concluídas e haja mudança de status */
                return true;
            }else if (($aux == count($count_tasks) || $status == 4) && $ticket[0]['status'] != $key_statuses[$status]) {
                /** Atualiza o status do tickets caso todas as tarefas estejam com o mesmo status ou o status
                 seja 4 - em progresso */
                return true;
            }
        }else if (count($count_tasks) == 1){
            return true;
        }
        return false;
    }

    public function save_task_status($status, $task_id)
    {
        $this->db
            ->insert('tbltasktimestatus', array(
    ...
    public function get_task_timestatus($status, $taskid)
    {
        return $this->db
            ->select('datetime')
    ...
    public function mark_as($status, $task_id)
    {
        ...
        if ($_task->status == 5) {
            ...
            if ($status == 5) {
                ...
                $this->insert_reply_task($status, $task_id);
                $this->db->where('id', $task_id);
                $this->db->update('tblstafftasks', array(
                    'status' => $status
                ));
                if ($this->db->affected_rows() > 0) {
                    $this->save_task_status($status, $task_id);
    ...
    public function insert_reply_task($status, $task_id){
        $this->db->select('status, rel_id');
        $this->db->where('id', $task_id);
        $_task = $this->db->get('tblstafftasks')->row();

        $status_de = get_task_status_by_id(get_status($_task->status)->ticketstatusid)['name'];
        $status_para = get_task_status_by_id(get_status($status)->ticketstatusid)['name'];
        $nome = $this->staff_model->getc(get_staff_user_id());

        if($status != self::EM_PROGRESSO) {

            $insert_status_id = $this->add_task_comment(array(
                'taskid' => $task_id,
                'content' => 'Status da tarefa alterado de "' . $status_de . '" para "' . $status_para . '" por: [ ' . $nome->firstname . ' ' . $nome->lastname . ' ]',
            ));
        }
        logActivity('Status da tarefa alterado de [' . $status_de . '] para [' . $status_para . '] por: [ ' . $nome->firstname . ' ' . $nome->lastname . ' , staffid = '. get_staff_user_id() .' ]');
    }
    ...
    public function unmark_complete($id, $force_to_status = false)
    {
        ...
        $this->insert_reply_task($status, $id);
        $this->db->where('id', $id);
        $this->db->update('tblstafftasks', array(
            'datefinished' => null,
            'status' => $status
        ...
        if ($this->db->affected_rows() > 0) {
            $this->save_task_status($status, $id);
            $task = $this->get($id);
            if ($task->rel_type == 'project') {
    ...
    {
        if ($this->input->post("tipo_parada") && ($this->input->post("tipo_parada") != "1")) {
            $this->add_task_comment(array("content" => "Parada relacionado: " . _l($this->input->post("tipo_parada")) . " #" . $this->input->post("rel_id") . "<br/>" . $note, "taskid" => $task_id));
        }
