...
class Staff_model extends CRM_Model
{
    ...
    public function get_staff($where){
        $atendimentos = $this->db->query("SELECT DISTINCT CONCAT(tblstaff.firstname,' ',tblstaff.lastname) 
        AS name,staffid FROM tbltickets INNER JOIN tblstaff ON tbltickets.assigned = tblstaff.staffid
        WHERE tblstaff.partner_id = ".get_staff_partner_id(). $where ."  ORDER BY 2 DESC ")
            ->result_array();
        foreach ($atendimentos as $key => $atendimento){
            if($atendimento['staffid']){
                $aux = $this->db->query("SELECT COUNT(*) AS numatt FROM tbltickets 
                WHERE tbltickets.assigned = ".$atendimento['staffid'] . $where .
                " and tbltickets.status in (3, 5)")->result_array();
                $atendimentos[$key]['numatt'] =  $aux[0]['numatt'];
            }
        }

        usort($atendimentos, function($a, $b){
            return $b['numatt'] - $a['numatt'];
        });
     ...
     * @param int|string $active Optional get all active or inactive
     * @param array $where
     * @param bool $email
     * @param bool $partner
     * @return mixed if id is passed return object else array
     */
    public function get($id = '', $active = '', $where = array(), $email = false, $partner = false)
    {
        if($partner)
            $this->db->where('partner_id', get_staff_partner_id());
        ...
        return $this->db
            ->join('tblstaffdepartments', 'tblstaffdepartments.staffid = tblstaff.staffid')
            ->order_by('firstname', 'desc')
            ->get('tblstaff')->result_array();
    }

    /**
     * Get staff member/s corrigido umas tretas
     * @param  mixed $id Optional - staff id
     * @param int|string $active Optional get all active or inactive
     * @param array $where
     * @param bool $email
     ...
     * @return mixed if id is passed return object else array
     */
    public function getc($id = '', $active = '', $where = array(), $email = false, $partner = false, $where_in = false)
    {
        if($partner)
            $this->db->where('partner_id', get_staff_partner_id());
    ...
    public function getSupp($op = false)
    {
       $part = get_staff_partner_id();
       $this->db
           ...
           ->where("active", 1);
       if($op)
           $this->db->where("partner_id", $part);
       if(PAINEL == QUANTUM)
           $this->db->where("role IN (4,5)");
       else if(PAINEL == INORTE)
    ...
    public function add($data)
    {
        ...
        if(isset($data['doc_cnh_isenabled']) && $data['doc_cnh_isenabled'] == "on")
        {
            $data['doc_cnh_isenabled'] = 1;
            $data['doc_cnh_validade'] = to_sql_date($data['doc_cnh_validade']);
        }
        else
        {
            $data['doc_cnh_isenabled'] = 0;
            $data['doc_cnh_num'] = NULL;
            $data['doc_cnh_categorycnh'] = NULL;
            $data['doc_cnh_validade'] = NULL;
        }
...
//        $where = " WHERE partner_id != 0 ";
        ...
        if(is_partner())
        {
            $where = "WHERE partner_id = ".get_staff_partner_id();
        }
        return $this->db
