...
<div id="wrapper">
                    ...
                    <div class="panel-body">
                        <ul class="nav nav-tabs" role="tablist">
                            ...
                            <?php if(!is_partner()){ ?>
                                ...
                                <li role="presentation">
                                    <a href="#tab_staff_documentation" aria-controls="tab_staff_documentation" role="tab"
                                       data-toggle="tab">
                                        Documentação
                                    </a>
                                </li>
                        ...
                        <div class="tab-content">
                            <div role="tabpanel" class="tab-pane active" id="tab_staff_profile">
                                ...
                                <?php $value = (isset($member) ? $member->email_sec : ''); ?>
                                <?php echo render_input('email_sec', 'Email Secundário', $value, 'email', array('autocomplete' => 'off')); ?>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <?php
                                        ...
                                        if(isset($member) && $member->staffid)
                                            $selected = get_staff_partner_id($member->staffid);
                                        echo render_select('partner_id', $partners, array('partner_id', 'firstname'), 'Parceiro', $selected, array(), array(), '', '', false);
                                    ?>
                                </div>
                        ...
                        <a href="#" class="generate_password" onclick="generatePassword(this);return false;"><i
                            ...
                            <div role="tabpanel" class="tab-pane" id="tab_staff_documentation">
                                <h4 class="font-medium mbot15 bold">Habilitação</h4>
                                <hr>
                                <div class="checkbox checkbox-primary">
                                    <?php
                                        $isenabled = '';
                                        $hab = "docsha.hide();";
                                        if (isset($member)) {
                                            if ($member->doc_cnh_isenabled == 1) {
                                                $isenabled = ' checked';
                                                $hab = "docsha.show();";
                                            }
                                        }
                                    ?>
                                    <input type="checkbox" name="doc_cnh_isenabled"
                                           id="doc_cnh_isenabled" <?php echo $isenabled; ?>>
                                    <label for="doc_cnh_isenabled">Possui Habilitação!</label>
                                </div>
                                <div id="docshabvi">
                                    <?php $value = (isset($member) ? $member->doc_cnh_num : ''); ?>
                                    <?php echo render_input('doc_cnh_num', 'Numero da CNH', $value); ?>
                                    <?php $value = (isset($member) ? $member->doc_cnh_categorycnh : ''); $cate = array(array("tipo"=>"A"),array("tipo"=>"B"),array("tipo"=>"AB"),array("tipo"=>"C"),array("tipo"=>"D"));?>
                                    <?php echo render_select('doc_cnh_categorycnh', $cate, array("tipo","tipo"), 'Categoria', $value); ?>
                                    <?php
                                        if(isset($member))
                                        {
                                            $date = date_create($member->doc_cnh_validade);
                                            $value = date_format($date, 'd/m/Y');
                                        }
                                        else
                                            $value = "";
                                    ?>
                                    <?php echo render_date_input('doc_cnh_validade', 'Validade', $value); ?>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table dt-table">
                                        ...
                                        <?php foreach($tickets as $ticket){ ?>
                                            <tr>
                                                <td>
                                                    <?php echo '<a href="'.APP_BASE_URL."admin/tickets/ticket/".$ticket['ticketid'].'">'.$ticket['ticketid']; ?></a>
                                                </td>
                                                <td>
                                                    <?php echo '<a href="'.APP_BASE_URL."admin/tickets/ticket/".$ticket['ticketid'].'">'.$ticket['subject']; ?></a>
                                                </td>
                                                <td>
                                                    <?php echo $ticket['company']; ?>
                                                </td>
                                                <td>
                                                    <span class="label inline-block" style="border:1px solid <?php echo $ticket['statuscolor']; ?>; color:<?php echo $ticket['statuscolor']; ?>"><?php echo ticket_status_translate($ticket['status']); ?></span>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                        </tbody>
    ...
    <?php init_tail(); ?>
    <script>
        var docsha = $('#docshabvi');
        <?php echo $hab;?>
        $('#doc_cnh_isenabled').click(function() {
            if($('#doc_cnh_isenabled')[0].checked)
                docsha.show();
            else
                docsha.hide();
        });
