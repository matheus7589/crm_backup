...
class Reports_model extends CRM_Model
{
    ...
    public function get_desempenho($where = "1=1")
    {
        $desempenho = new stdClass();
        if(strpos($where,"vehicleid") == true) {
            $kilometragem = $this->db->select("kilometragem")->order_by("data","ASC")->where($where)->get("tblfleetsupply")->result_array();
            if (count($kilometragem) <= 2) {
                $desempenho->km = "São nescessários no mínimo 3 abastecimentos.";
                $desempenho->label = "danger";
            } else {
                $litros = $this->db->select("SUM(litro) as litro")->where($where)->get("tblfleetsupply")->result_array();
                $distancia = intval(end($kilometragem)['kilometragem']) - intval($kilometragem[0]['kilometragem']);
                $desempenhoo = round($distancia / $litros[0]['litro'], 2);
                $desempenho->km = $desempenhoo . " Km/L";
                if ($desempenhoo >= 13)
                    $desempenho->label = "success";
                else if ($desempenhoo >= 8 && $desempenhoo < 13)
                    $desempenho->label = "info";
                else if ($desempenhoo >= 4 && $desempenhoo < 8)
                    $desempenho->label = "warning";
                else
                    $desempenho->label = "danger";
            }
        }
        ...
        {
            $desempenho->km = "Selecione um veículo";
            $desempenho->label = "info";
        }
        return $desempenho;
    }
