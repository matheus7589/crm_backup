      ...
      <div class="col-md-12">
       <div class="panel_s">
        <div class="panel-body _buttons">
         ...
         <?php } ?>
         <a href="<?php echo admin_url('knowledge_base/manage_groups'); ?>" class="btn btn-info mright5">Grupos</a>
       ...
       <div class="panel-body">
        <div class="row">
         ...
         <div role="tabpanel" class="tab-pane kb-kan-ban kan-ban-tab" id="kan-ban">
           ...
           {
               if (!empty($group['departments'])) {
                   $this->db->query("SELECT * FROM `tblstaffdepartments` WHERE `staffid` = " . get_staff_user_id() . " AND `departmentid` IN (" . $group['departments'] . ")");
               }
               if ($this->db->affected_rows() > 0 || is_admin())
               {
             $kanban_colors = '';
   ...
   $(".container-fluid").sortable({
     ...
     update: function(event, ui) {
       ...
       $('.toggle-articles-list').on('click', function() {
         ...
         if (list_tab.hasClass('toggled') || list_tab.hasClass('active')) {
           list_tab.css('display', 'none').removeClass('toggled').removeClass('active');
