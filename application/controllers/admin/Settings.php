...
class Settings extends Admin_controller
{
    ...
    public function index()
    {
        ...
        if($view == 'update-db')
        {
            ...
            {
                if($this->updatedb_model->comandos($this->input->get("install"),true)["error"] == 0)
                    $this->db->query("UPDATE `tbldatabasehistory` SET `installed` = '1' WHERE `tbldatabasehistory`.`version` = ".$this->input->get("install"));
                header("location: ".admin_url("settings?group=update-db"));
            }
            if($this->input->get("uninstall") != "")
            {
                if($this->updatedb_model->comandos($this->input->get("uninstall"),false)["error"] == 0)
                    $this->db->query("UPDATE `tbldatabasehistory` SET `installed` = '0' WHERE `tbldatabasehistory`.`version` = ".$this->input->get("uninstall"));
                header("location: ".admin_url("settings?group=update-db"));
            }
            if($this->input->get("reinstall") != "")
            {
                if($this->updatedb_model->comandos($this->input->get("reinstall"),true)["error"] == 0)
                    $this->db->query("UPDATE `tbldatabasehistory` SET `installed` = '0' WHERE `tbldatabasehistory`.`version` = ".$this->input->get("reinstall"));
                if($this->updatedb_model->comandos($this->input->get("reinstall"),false)["error"] == 0)
                    $this->db->query("UPDATE `tbldatabasehistory` SET `installed` = '1' WHERE `tbldatabasehistory`.`version` = ".$this->input->get("reinstall"));
                header("location: ".admin_url("settings?group=update-db"));
            }
            if($this->input->get("update")){
                $this->updatedb_model->update();
                header("location: ".admin_url("settings?group=update-db"));
            }
            $data['dbcurrentversion'] = get_option("db_version");
            $data['dbversion'] = $this->updatedb_model->last_version();
            $data['allversions'] = $this->updatedb_model->versions();
            $data['installed_versions'] = $this->db->order_by("updateid DESC")->get("tbldatabasehistory")->result_array();
        }
