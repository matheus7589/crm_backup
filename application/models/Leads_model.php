...
class Leads_model extends CRM_Model
{
    ...
    public function add($data)
    {
        ...
        //Eu coloquei assim pq tava dando um 'errozinho' de variável não definida
        $temp_cont = $data['nextcontact'];
        $data['nextcontact'] = null;

        if(isset($temp_cont))
        {
            if ($temp_cont != '')
                $data['nextcontact'] = to_sql_date($temp_cont, true);
        }
        ...
        if ($insert_id) {
            if(isset($temp_cont))
            {
                if ($temp_cont != '')
                {
                    $data_ev["title"] = "Contato Lead #".$insert_id;
                    $data_ev["description"] = "Retornar contato [".$data['name']."]";
                    $data_ev["userid"] = get_staff_user_id();
                    $data_ev["start"] = $data['nextcontact'];
                    $data_ev["public"] = 0;
                    $data_ev["color"] = "#84C529";
                    $this->db->insert("tblevents", $data_ev);
                }
            }
    ...
    public function update($data, $id)
    {
        ...
        if($data['nextcontact'] != '') {
            $data['nextcontact'] = \Carbon\Carbon::createFromFormat('d/m/Y H:i:s', $data['nextcontact'])
                ->format('Y-m-d H:i:s');
        }else{
            $data['nextcontact'] = null;
        }
