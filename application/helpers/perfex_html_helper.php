...
function app_stylesheet($path,$filename){
    ...
    if(get_option('use_minified_files') == 1 && ENVIRONMENT == "production"){
...
function app_script($path,$filename){
    ...
    if(ENVIRONMENT == 'production'){
        ...
        if(file_exists($path.'/min/'.$filename)){
 ...
 * @return string
 */
function render_input($name, $label = '', $value = '', $type = 'text', $input_attrs = array(), $form_group_attr = array(), $form_group_class = '', $input_class = '', $label_attr = array())
{
    $input            = '';
    $_form_group_attr = '';
    $_input_attrs     = '';
    ...
    if ($label != '') {
        $input .= '<label ' . $_label_attrs . ' for="' . $name . '" class="control-label">' . _l($label,'',false) . '</label>';
    }
 ...
 * @return string
 */
function render_select($name, $options, $option_attrs = array(), $label = '', $selected = '', $select_attrs = array(), $form_group_attr = array(), $form_group_class = '', $select_class = '', $include_blank = true, $forcezero = false)
{
    ...
    foreach ($options as $option) {
        ...
        if($key == 0 and $forcezero)
            $key = strval(0);
        $data_sub_text = '';
        if (!is_array($selected)) {
            if ($selected != '') {
                if (strval($selected) == strval($key)) {
                    $_selected = ' selected';
                }
            }
        } else {
...
function init_head($aside = true)
{
    ...
    $CI->load->view('admin/includes/modals/master_notifications');
    $CI->load->view('admin/includes/modals/pending_tickets');
    if ($aside == true) {
        if(is_partner())
        {
            $CI->load->view('admin/includes/aside-partner');
        }
        else
        {
    ...
    ?> table-tickets">
        ...
        <tr>
            ...
            <th class="ticket_created_column">
                ...
                ?></th>
            <th <?php if(get_option('show_task_in_ticket_table') == 0){echo ' class="not_visible"'; }?>><?php
                echo _l('tasks');
                ?></th>
            <?php
      ...
      var options = {
        ajax: {
          ...
          data: function () {
            ...
            data.rel_id = '';
            if(PAINEL == INORTE) {
                data.contact_userid = $('select[name="clientid"]').val();
            }
            data.q = '{{{q}}}';
        ...
        preprocessData: function(processData){
          ...
          for(var i = 0; i < len; i++){
            ...
            //removendo esse if, o nome da empresa não aparece na busca do ajaxSearch
...
function check_for_links($ret)
{
    ...
    $ret = preg_replace("#(<a target=.'_blank'. ( [^>]+?>|>))<a [^>]+?>([^>]+?)</a></a>#i", "$1$3</a>", $ret);
    if(PAINEL == INORTE) {
        $ret = str_replace("http://www.inorte.ddns.net/crm/", site_url(), $ret);
        $ret = str_replace("http://inorte.ddns.net/crm/", site_url(), $ret);
        $ret = str_replace("http://10.1.1.64/crm/", site_url(), $ret);
        $ret = str_replace("https://crm.inorte.com.br/", site_url(), $ret);
        $ret = str_replace("http://crm.inorte.com.br/", site_url(), $ret);
        $ret = str_replace("crm.inorte.com.br/", site_url(), $ret);
    }
    $ret = trim($ret);
    return $ret;
}
...
function get_nota_formated($nota,$etiqueta = false)
{
    $CI =& get_instance();
    $nota = $CI->db->where("nota",$nota)->get("nota_atendimento")->row();
    if($etiqueta)
        return '&nbsp;<i class="fa fa-bookmark text-'.$nota->span_color.'" data-toggle="tooltip" data-original-title="'.$nota->descricao.'"></i>&nbsp;';
    else
        return '<span class="col-md-12 label label-'.$nota->span_color.'" style="font-size: 100%; background: inherit;">'.$nota->descricao.'</span>';
}
function render_player($id,$key = '',$admin = false)
{
    $CI =& get_instance();
    $CI->load->model('movies_model');

    if(is_numeric($id)) {
        $media = $CI->movies_model->get_movie($id);
        if(isset($media)) {
            if($CI->input->get('key'))
                $key = $CI->input->get('key');
            $perm = has_permission_video($id, $key);
//                print_r($media->filename);
            if($media->is_url == 1)
            {
                echo '<style>body{margin:0px;}</style><iframe width="100%" height="100%" src="'.$media->filename.'?autoplay=1" frameBorder="0"></iframe>';
                return true;
            }
            if ($perm) {
                error_reporting(0);
                $local_file = str_replace("\\", '/', FCPATH . "uploads/movies/" . $media->filename);
                $download_file = str_replace("\\", '/', FCPATH . "uploads/movies/" . $media->filename);
            ...
            else if($media->perm_key != NULL) {
//                echo form_open(admin_url("movies/player/".$id));
                if($admin)
                    echo "<form method='get' action='".admin_url("movies/player/".$id)."'>";
                else
                    echo "<form method='get' action='".base_url("movies/player/".$id)."'>";
                echo render_input('key', 'Código de acesso.');
                echo "<input type='submit'>";
                echo form_close();
                return true;
            }
            else
                set_alert("danger","Não há permição para visualizar o vídeo");
        }
        else
            set_alert("danger","Vídeo não encontrado");
    }
    if($admin)
...
function render_disable_ticket_fields($ticket = null, $client = null){

    if($client != null){
        $CI =& get_instance();
        $CI->load->model('clients_model');
        $p_contact = $CI->clients_model->get_contact($client->primary_contact);
    }
    $retorno = '<div class="row">';
    $retorno .= '<div class="col-md-4">';
    $retorno .= render_input('fantasia', 'Nome Fantasia', $ticket->nome_fantasia ?? '', 'text', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '<div class="col-md-4">';
    $retorno .= render_input('telefoneEmpresa', 'Telefone Empresa', $ticket->telEmpresa ?? '', 'text', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '<div class="col-md-4">';
    $retorno .= render_input('celEmpresa', 'Celular Empresa', $client->cellphone ?? '', 'text', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '</div>';
    $retorno .= '<div class="row">';
    $retorno .= '<div class="col-md-4">';
    $retorno .= render_input('to', 'Contato Principal', $p_contact->firstname ?? '', 'text', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '<div class="col-md-4">';
    $retorno .= render_input('emailPrimary', 'E-mail Principal', $p_contact->email ?? '', 'email', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '<div class="col-md-4">';
    $retorno .= render_input('phonePrimary', 'Telefone Principal', $p_contact->phonenumber ?? '', 'text', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '</div>';
    $retorno .= '<div class="row">';
    $retorno .= '<div class="col-md-4">';
    $retorno .= render_input('contact_now', 'Contato Selecionado', $ticket->nome_cliente ?? '', 'text', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '<div class="col-md-4">';
    if($ticket != null) {
        if ($ticket->userid != 0) {
    ...
    $retorno .= render_input('email_contact_now', 'E-mail Selecionado', $email ?? '', 'email', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '<div class="col-md-4">';
    $retorno .= render_input('phone_contact_now', 'Telefone Selecionado', $ticket->telCliente ?? '', 'text', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '</div>';
    $retorno .= '<div class="row">';
    $retorno .= '<div class="col-md-6">';
    $retorno .= render_input('revenda', 'Parceiro/Revenda', ($client != null) ? get_partner_name($client->partner_id) : '', 'text', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '<div class="col-md-6">';
    $retorno .= render_input('ultimo_atendimento', 'Último Atendimento', $ticket->atendimento ?? '', 'text', array('disabled' => true));
    $retorno .= '</div>';
    $retorno .= '</div>';

    $retorno .= render_input_tags($ticket);


    if((is_admin() || has_permission('avaliacao_atendimento', '', 'view')) && !empty($ticket)){
        $retorno .= '<div class="row">';
        $retorno .= '<div class="col-md-6">';
        $retorno .= '<div class="form-group">';
        $retorno .= '<a onclick="atendimento_tecnico(); return false;" class="btn btn-info">';
        if(PAINEL == INORTE)
            $retorno .= "Avaliação do Técnico";
        else
            $retorno .= "Avaliação do Atendimento";
        $retorno .= '</a>';
        $label = isset($ticket->nota_atendimento) ? 'success' : 'danger';
        $retorno .= '<span class = "label label-'. $label .' mleft10" style="font-size: 100%; background: inherit; border-width: 2px;">';
        $retorno .= '<b>';
        if($label == 'danger') $retorno .= 'Não ';
        $retorno .= 'Avaliado';
        $retorno .= '</b>';
        $retorno .= '</span>';
        $retorno .= '</div>';
        $retorno .= '</div>';
        $retorno .= '<div class="col-md-6">Versão do Cliente: <span class="label label-success"> ' . get_version_by_user_id($ticket->userid) . '</span></div>';
        $retorno .= '</div>';
    }
    $retorno .= '</div>';
...
function render_input_tags($ticket){
    $return = '<div class="form-group" >';
    $return .= '<label for="tags" class="control-label"><i class="fa fa-tag" aria-hidden="true"></i>';
    $return .= _l('tags');
    $return .= '</label><input type="text" class="tagsinput" id="tags" name="tags" value="';
    if (!empty($ticket))
        $return .= prep_tags_input(get_tags_in($ticket->ticketid, 'ticket'));
    $return .= '" data-role="tagsinput"></div>';
