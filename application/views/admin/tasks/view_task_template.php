   ...
   <div class="row">
      <div class="col-md-8 task-single-col-left">
         ...
         <p class="no-margin pull-left">
            <a href="#" data-toggle="popover" data-html="true" onClick="$(function(){$('#motivo').selectpicker('refresh'); inicia_minicampo();});" data-trigger="manual" data-title="<?php echo _l('note'); ?>" data-content='<?php echo render_select("motivo",array(array("tipo"=>"Intervalo/Fim Expediente","value"=>"1"),array("tipo"=>"Atendimento","value"=>"ticket"),array("tipo"=>"Outra Tarefa","value"=>"tasks")),array("value","tipo"),"Motivo","",array(),array(),"","", false).'<div class="form-group hide" id="rel_id_wrapper"><label for="rel_id" class="control-label"><span class="rel_id_label"></span></label><div id="rel_id_select"><select name="rel_id" id="rel_id" class="ajax-sesarch" data-width="100%" data-live-search="true" data-none-selected-text="'._l('dropdown_non_selected_tex').'"></select></div></div>'.render_textarea('timesheet_note',"","",array("placeholder"=>"Observação")); ?><button type="button" onclick="timer_action(this,<?php echo $task->id; ?>,<?php echo $this->tasks_model->get_last_timer($task->id)->id; ?>);" class="btn btn-info btn-xs"><?php echo _l('save'); ?></button>' class="btn mbot10 btn-danger<?php if(!$is_assigned){echo ' disabled';} ?>" onclick="return false;" id="bntstoptask">
         ...
         <div class="tasks-comments inline-block full-width" <?php if(count($task->comments) == 0){echo 'style="display:none"';} ?>>
            ...
            <button type="button" class="btn btn-info mtop30 pull-right" autocomplete="off" onclick="add_task_comment('<?php echo $task->id; ?>','undefined');">
            ...
            <div id="task-comments" class="mtop10">
               <?php
                  ...
                  foreach ($task->comments as $comment) {
                   ...
                   $comments .= '<small class="mtop5 text-muted"><a data-task-comment-href-id="'.$comment['id'].'" href="'.admin_url('tasks/view/'.$task->id).'#comment_'.$comment['id'].'" class="task-date-as-comment-id">' . Carbon\Carbon::parse($comment['dateadded'])->format("d/m/y H:i") . '</a></small>';
                  ...
                  $comments .= '<div class="comment-content mtop10 fonte">'.check_for_links($comment['content']) . '</div>';
                  $comments .= '</div>';
                  if ($i >= 0 && $i != $len - 1) {
      ...
      <div class="col-md-4 task-single-col-right">
         ...
         <?php //print_r($task); ?>
         <h4 class="task-info-heading"><?php echo _l('task_info') . ' #' . $task->id; ?></h4>
         <div class="clearfix"></div>
         <h5 class="no-mtop task-info-created">
            ...
            <br />
            <?php if($task->datefinished != '0000-00-00 00:00:00') { ?>
              <small class="text-dark"><?php echo 'Finalizado em ' . _dt($task->datefinished); ?></small>
            <?php }?>
         </h5>
                        ...
                        <?php
                           ...
                           foreach($task_single_mark_as_statuses as $status){ ?>
                              <?php if($task->status != $status['id']){ ?>
                                 <li>
                                    <a href="#" onclick="verify_task_assigned(this, <?php echo $status['id']; ?>,<?php echo $task->id; ?>, undefined); return false;">
            ...
            foreach ($staff as $assignee)
            {
                if (total_rows('tblstafftaskassignees', array('staffid' => $assignee['staffid'],'taskid' => $task->id)) == 0)
                {
                    if(PAINEL == INORTE)
                    {
                        ...
                        {
                            if (total_rows('tblprojectmembers', array('project_id' => $task->rel_id,'staff_id' => $assignee['staffid'])) == 0)
                            {
                                continue;
                            }
                        }
                        if($assignee['departmentid'] == 3)
                            $options .= '<option value="' . $assignee['staffid'] . '">' . get_staff_full_name($assignee['staffid']) . '</option>';
                    }
                    else if(PAINEL == QUANTUM && $assignee['staffid'] != $ultimo)
                    {
                        $options .= '<option value="' . $assignee['staffid'] . '">' . get_staff_full_name($assignee['staffid']) . '</option>';
         ...
         <?php
            ...
            {
                if (total_rows('tblstafftasksfollowers', array('staffid' => $follower['staffid'],'taskid' => $task->id)) == 0)
                {
                    if(PAINEL == INORTE)
                    {
                        if(($follower['departmentid'] == 1 && ($follower['role'] == 4 || $follower['role'] == 6) && $follower['partner_cnpj'] == null) || $follower['admin'] == 1 && $follower['role'] != 5 && $follower['role'] != 1 )
                        {
                            $options .= '<option value="' . $follower['staffid'] . '">' . get_staff_full_name($follower['staffid']) . '</option>';
                        }
                    }
                    else if(PAINEL == QUANTUM && $follower['staffid'] != $ultimo)
                    {
                        $options .= '<option value="' . $follower['staffid'] . '">' . get_staff_full_name($follower['staffid']) . '</option>';
                        $ultimo = $follower['staffid'];
                    }
                }
            }
            echo $options;
...
<script>
  ...
  function add_task_comment(task_id, comment) {
      var data = {};
      ...
      if(data.content.length < 20){
        alert("Um comentário deve ter, pelo menos, 20 caracteres!");
      }else{
        $.post(admin_url + 'tasks/add_task_comment', data).done(function(response) {
          init_task_modal(task_id, 1);
        });
      }

  }

  function add_task_comment_custom(task_id, comment_status) {
      var data = {};
      var name = '<?php echo str_replace("'", "&#39;", $task->name); ?>';

      // console.log(name);
      data.content = "Iniciando Tarefa " + name;
      data.taskid = task_id;
      if(data.content.length < 20){
        alert("Um comentário deve ter, pelo menos, 20 caracteres!");
      }else{
        $.post(admin_url + 'tasks/add_task_comment', data).done(function(response) {
          init_task_modal(task_id, comment_status);
        });
      }

  }

  function verify_task_assigned(isto, status, task_id, url){

    var next_task = '<?php $subject = str_replace("'", "&#39;", $task->name); if(isset($subject)){ echo $subject;} ?>';

    $.post(admin_url + 'tasks/verify_tasks').done(function(response){
      response = JSON.parse(response);
      if(response.verify == 1){
        var current_task = response.tasks[0].name;
        var current_id = response.tasks[0].id;
        if(status == 4){
          if(confirm("A tarefa '" + current_task + "' está em andamento. Deseja parar esta tarefa e iniciar a tarefa '" + next_task + "'?") == true){
            stop_current_task(current_id, current_task, next_task);
              window.setTimeout(function() {
                change(isto, status, task_id, url);
                }, 3000);
  ...
  function stop_current_task(task_id, current, next){
    mark_paused(task_id);
    add_task_comment(task_id, 'Parando tarefa "' + current + '" para inicializar a tarefa "' + next + '", de maior prioridade!');
  }

  function change(isto, status, task_id, url) {
    var is_admin = '<?php echo is_admin(); ?>';
    var aux;
    if(is_admin){
      aux = 1;
    }else{
      aux = '<?php echo $task->comment; ?>';
    }

    var statusnow = '<?php echo $task->status; ?>';

    if(statusnow == 1 && status == 4){
      task_mark_as(status, task_id, url);
      add_task_comment_custom(task_id, 0);
      timer_action(this,task_id);
    }else if(aux == 1 && status == 1){
      var last_timer = '<?php $timer = $this->tasks_model->get_last_timer($task->id); if(isset($timer->id)){ echo $timer->id;} ?>';
      timer_action(isto, task_id, last_timer);
      task_mark_as(status, task_id, url);
    }else if(aux == 1 && status == 5){
      ...
      alert("não pode mudar o Status da tarefa sem antes adicionar um comentário!");
    }
}

function inicia_minicampo()
{
    var _rel_id = $('#rel_id'),
        _rel_type = $("#motivo"),
