...
function get_version_by_user_id($id)
{
    $CI =& get_instance();
    $CI->db->select('userid, versao');
    $CI->db->where('userid', $id);
    $version = $CI->db->get('tblversoes')->row();
    if ($version) {
        return $version->versao;
    }

    return 'Não possui versão salva';
}
...
function get_companies($contactid){
    $CI =& get_instance();
    $CI->db->select('company, tblclients.userid as companyid, tblclientscontacts.id as clientscontactsid');
    $CI->db->join('tblclientscontacts', 'tblclientscontacts.contactid = tblcontacts.id', 'left');
    $CI->db->join('tblclients', 'tblclients.userid = tblclientscontacts.clientid', 'left');
    $CI->db->where('tblcontacts.id', $contactid);
    $CI->db->where('tblclients.active', 1);

    $companies = $CI->db->get('tblcontacts')->result_array();
...
{
    if(check_menu_item($options['id']) != false)
        return false;
...
/**
 * Função para remover item do main_menu
 * @param string $id
 * @return bool
 */
function remove_main_menu_item($id = '')
{
    $items = json_decode(get_option('aside_menu_active'))->aside_menu_active;
    $newitems = array();
    $mensagem = "";
    foreach ($items as $key => $item) {
        if(isset($item->children)){
            $filho = array();
            foreach ($items[$key]->children as $keyc => $subitems){
                if($subitems->id != $id)
                    array_push($filho, $subitems);
                else
                    $mensagem .= "Desenvolvimento[Item do SubMenu removido - ".json_encode($subitems)."]";
            }
            $items[$key]->children = $filho;
        }
        if($item->id != $id)
            array_push($newitems,$item);
        else
            $mensagem .= "Desenvolvimento[Item do Menu removido - ".json_encode($item)."]";
    }
    $menu = json_encode(array("aside_menu_active" => $newitems));
    if(md5(get_option('aside_menu_active')) != md5($menu)) {
        update_option('aside_menu_active',$menu);
        logActivity($mensagem);
        return true;
    }
    return false;
}
 ...
 * @return bool
 */
function remove_setup_menu_item($id = '')
{
    $items = json_decode(get_option('setup_menu_active'))->setup_menu_active;
    $newitems = array();
    $mensagem = "";
    foreach ($items as $key => $item) {
        if(isset($item->children)){
 ...
 * @param boolean $allstaffs
 * @param boolean $activeOnly
 * @return boolean
 */
function add_notification($values,$allstaffs = false,$activeOnly = true,$system = false)
{
    $CI =& get_instance();
 ...
 * @param  mixed $sIndexColumn       main column in table for better performing
 ...
 * @param string $custom custom result
 * @return array
 */
function data_tables_init($aColumns, $sIndexColumn, $sTable, $join = array(), $where = array(), $additionalSelect = array(), $sGroupBy = '', $custom = '')
{
    $CI =& get_instance();
...
function get_relation_data($type, $rel_id = '', $connection_type = '', $connection_id = '')
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        $parner = "";
        if(get_staff_partner_id() != 0)
            $parner = " AND partner_id = ".get_staff_partner_id();
        $data = $CI->clients_model->get($rel_id, $where_clients.$parner);

    } elseif($type == 'contact' || $type == 'contacts'){
        if ($rel_id != '') {
            $data = $CI->clients_model->get_contact_related_clientcontact($rel_id);
        } else {
...
//                echo $CI->input->post('contact_userid');
                $where_contacts .= ' AND tblclientscontacts.clientid='.$CI->input->post('contact_userid');
            }
        ...
        if ($rel_id != '') {
            ...
            $search = $CI->misc_model->_search_tasks($q);
            $data   = $search['result'];
        }
    } elseif ($type == 'equipments') {
        if ($rel_id != '') {
            $data = $CI->db->select("CONCAT('#',equipinid,' ',description) as name,'' as link, tipo as subtext,equipinid as id")->where('equipinid = "'.$q.'"')->get("tblequipments_in")->row();
        } else {
            $data = $CI->db->select("CONCAT('#',equipinid,' - ',description) as name,'' as link, tipo as subtext,equipinid as id")->where('(description LIKE "%'.$q.'%" OR equipinid LIKE "%'.$q.'%") AND flag_out = 0 AND status = 1')->get("tblequipments_in")->result_array();
        }
    } elseif ($type == 'other') {
            if ($rel_id != '') {
                $data = $CI->db->select("name,link,subtext,relationid as id")->where('relationid = "'.$q.'"')->get("tblotherrelations")->row();
            } else {
                $data = $CI->db->select("name,link,subtext,relationid as id")->where('name LIKE "%'.$q.'%" OR subtext LIKE "%'.$q.'%"')->get("tblotherrelations")->result_array();
            }
    } elseif ($type == 'knowledge_base'){
        if ($rel_id != '') {
            $data = $CI->db->select(" subject as name, slug as link, '' as subtext, articleid as id")->where('articleid = "'.$rel_id.'"')->get("tblknowledgebase")->row();
        } else {
            $data = $CI->db->select(" subject as name, slug as link, '' as subtext, articleid as id")->where('(subject LIKE "%'.$q.'%" OR articleid LIKE "%'.$q.'%")')->get("tblknowledgebase")->result_array();
        }
    }
...
//    var_dump($relation);
    if ($type == 'customer' || $type == 'customers') {
        ...
        if (is_array($relation)) {
            ...
            $id   = isset($relation['clientcontactid']) ? $relation['clientcontactid'] : $relation['id'];
//            $id   = $relation['id'];
            $name = $relation['firstname'] . ' ' . $relation['lastname'];
        } else {
            $userid = $relation->userid;
            $id   = ($relation->clientcontactid) ? $relation->clientcontactid : $relation->id;
//            $id   = $relation->id;
            $name = $relation->firstname . ' ' .$relation->lastname;
        }
        ...
        $link = admin_url('knowledge_base/view/' . $slug);
    } else{
        if (is_array($relation)) {
            $id = $relation['id'];
            $name = $relation['name'];
...
/**
 * Fução para adicionar novo item ao relatório de mudanças
 * @param string $staffName
 * @param date $date
 * @param string $description
 * @param string $type | new/bug/note
 * @param string $version
 * @param string $KnowBaseSlug
 * @return boolean
 * */
function add_changelog_item($staffName,$date,$description,$type,$version,$KnowBaseSlug = NULL)
{
    $CI =& get_instance();
    $data['Autor'] = $staffName;
    if($date == "" || $date == NULL)
        $data['Data'] = Carbon\Carbon::now()->toDateString();
    else
        $data['Data'] = to_sql_date($date);
    $data['Titulo'] = $description;
    $data['Tipo'] = $type;
    $data['Version'] = $version;
    $data['moduleid_fk'] = 1;
    if($KnowBaseSlug != NULL)
        $data['slug'] = $KnowBaseSlug;
    $CI->db->insert("changelog",$data);
    if($CI->db->affected_rows() > 0)
        return true;
    return false;
}
 ...
 * @return bool|int
 */
function check_menu_item($id)
{
    $menu = get_option('aside_menu_active');
    $seach = '"id":"'.$id.'"';
    return strpos($menu,$seach);
}
