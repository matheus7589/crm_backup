...
class Misc_model extends CRM_Model
{
    ...
    public function perform_search($q)
    {
        ...
        if ($have_assigned_customers || $have_permission_customers_view) {
            ...
            $this->db->join('tblclientscontacts', 'tblclientscontacts.clientid = tblclients.userid', 'left');
            $this->db->join('tblcountries', 'tblcountries.country_id = tblclients.country', 'left');
            $this->db->join('tblcontacts', 'tblcontacts.userid = tblclientscontacts.clientid AND is_primary = 1', 'left');
            ...
            if ($have_assigned_customers && !$have_permission_customers_view) {
                if(isset($_SESSION['all_partners'])){
                    ...
                    $this->db->where('tblclients.userid IN (SELECT customer_id FROM tblcustomeradmins WHERE staff_id=' . get_staff_user_id() . ' and tblclients.partner_id = ' . get_staff_partner_id() . ')');
                }
            ...
            if(!isset($_SESSION['all_partners']))
                $this->db->where("tblclients.partner_id = ", $partner_id);
        ...
        if ($have_assigned_customers && !$have_permission_customers_view) {
            if(isset($_SESSION['all_partners'])){
                ...
                $where_contacts = 'tblcontacts.userid IN (SELECT customer_id FROM tblcustomeradmins WHERE staff_id=' . get_staff_user_id() . 'and tblclients.partner_id = ' . get_staff_partner_id() . ')';
            }
        ...
        $partner_search = $this->_search_partners($q, $limit);
        if (count($partner_search['result']) > 0) {
            $result[] = $partner_search;
        }
        ...
        // Staff tasks
        $this->db->select('tblstafftasks.*, tbltickets.ticketid, tbltickets.partner_id');
        ...
        if (!_startsWith($q, '#')) {
            $this->db->where('(tblstafftasks.name LIKE "%' . $q . '%" OR description LIKE "%' . $q . '%")');
        } else {
            $this->db->where('id IN
        ...
        $this->db->join('tbltickets', 'tbltickets.ticketid = tblstafftasks.rel_id AND rel_type = "ticket" ', 'RIGHT');

        if(!isset($_SESSION['all_partners']))
            $this->db->where('tbltickets.partner_id', $partner_id);

        $this->db->limit($limit);
        $this->db->order_by('tblstafftasks.name', 'ASC');
...
//        }
    ...
    public function _search_tickets($q, $limit = 0)
    {
        ...
        if (is_staff_member() || (!is_staff_member() && get_option('access_tickets_to_none_staff_members') == 1)) {
            ...
            $this->db->join('tblclientscontacts', 'tblclientscontacts.clientid = tbltickets.userid AND tblclientscontacts.contactid = tbltickets.contactid', 'left');
            $this->db->join('tblclients', 'tblclients.userid = tblclientscontacts.clientid', 'left');
            $this->db->join('tblcontacts', 'tblcontacts.id = tblclientscontacts.contactid', 'left');
    ...
    public function _search_contacts($q, $limit = 0, $where = '')
    {
        ...
        $aux = '';
        if(is_partner()) {
            $aux = " AND tblclients.partner_id = '" . get_staff_partner_id() . "' ";
        }
        ...
        if ($have_assigned_customers || $have_permission_customers_view) {
            ...
            $this->db->select('tblclientscontacts.id as clientcontactid');
            ...
            $this->db->where('(firstname LIKE "%' . $q . '%"
                ...
                OR title LIKE "%' . $q . '%"
                OR company LIKE "%' . $q . '%"
                OR tblclients.cnpj_or_cpf LIKE "%' . $q . '%"
                OR tblclients.social_reason LIKE "%' . $q . '%"
                ) ' . $aux  );
...
//            $this->db->group_by('tblclientscontacts.contactid');
...
//        }
    ...
    public function _search_partners($q, $limit = 0)
    {
        $result = array(
            'result' => array(),
            'type' => 'partner',
            'search_heading' => 'Parceiros'
        );
        ...
        if (has_permission('staff', '', 'view')) {
            ...
            $this->db->or_like('partner_cnpj', $q);
            $this->db->or_like("CONCAT(firstname, ' ', lastname)", $q, false);
            $this->db->or_like('email', $q);
