...
//    var_dump($tasks_filter_assignees);
    if(has_permission('tasks','','view')){
        foreach($tasks_filter_assignees as $tf_assignee){
            echo form_hidden('task_assigned_'.$tf_assignee['assigneeid'], $this->session->tempdata('assigned_task') !=  null && in_array($tf_assignee['assigneeid'], $this->session->tempdata('assigned_task')) ? true : false);
        }
    }
...
<div class="btn-group pull-right mleft4 mbot25 btn-with-tooltip-group _filter_data" data-toggle="tooltip" data-title="<?php echo _l('filter_by'); ?>">
   ...
   <ul class="dropdown-menu width300">
    ...
    <li class="divider"></li>
    <li class="dropdown-submenu pull-left <?php if($this->session->tempdata('assigned_task') != null) echo 'active' ; ?>">
       <a href="#" tabindex="-1"><?php echo _l('filter_by_assigned'); ?></a>
       <ul class="dropdown-menu dropdown-menu-left">
        <?php foreach($tasks_filter_assignees as $as){ ?>
        <li class="<?php if($this->session->tempdata('assigned_task') != null)
            if( in_array($as['assigneeid'], $this->session->tempdata('assigned_task'))) echo 'active'; ?>">
