...
<div class="modal fade<?php if(isset($task)){echo ' edit';} ?>" id="_task_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"<?php if($this->input->get('opened_from_lead_id')){echo 'data-lead-id='.$this->input->get('opened_from_lead_id'); } ?>>
  <div class="modal-dialog" role="document">
      ...
      <div class="modal-body">
        <div class="row">
          <div class="col-md-12">
            ...
            $rel_id = '';
            $replyid = '';
            if(isset($task) || ($this->input->get('rel_id') && $this->input->get('rel_type') && $this->input->get('replyid'))){
            ...
            <?php if(!isset($task)){ ?>
            <a href="#" class="pull-right btn btn-info" onclick="slideToggle('#new-task-attachments'); return false;">
            ...
            $rel_data = get_relation_data($rel_type,$rel_id);
             ...
             $valor = '';
             $pedacos = array();
            if(isset($rel_data->ticketid)){
              $pedacos = explode(" ", $rel_data->company);
              foreach($pedacos as $key => $pedaco){
                if($key > 1){
                  unset($pedacos[$key]);
                }
              }
              $valor = implode(" ", $pedacos);
            }
             ?>
             <?php $IdTicket = (isset($rel_data->ticketid) ? trim('#'. $rel_data->ticketid . ' - ' . $rel_data->subject . ' / ' . $valor) : ''); ?>
            <?php echo render_input('name','task_add_edit_subject', $IdTicket); ?>
    ...
    <?php 
    if($replyid != 'message'){
      $description = $this->tickets_model->get_single_ticket_reply($replyid);
      ...
      if(isset($rel_data)){
        $description = $rel_data->message;
      }else{
        $description = '';
      }
    }
    ?>
    <?php echo render_textarea('description','',(isset($task) ? $task->description : $description),array('rows'=>6,'placeholder'=>_l('task_add_description'),'data-task-ae-editor'=>true,'onclick'=>(!isset($task) || isset($task) && $task->description == '' ? 'init_editor(\'.tinymce-task\',{height:200,auto_focus: true});' : '')),array(),'no-mbot','tinymce-task'); ?>
  </div>
</div>
</div>
