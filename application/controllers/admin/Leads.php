...
class Leads extends Admin_controller
{
    ...
    public function index($id = '')
    {
        ...
        $data['staff'] = $this->staff_model->getc('', '', " (active = '1' AND `role` = '2') OR (admin = '1' AND active = '1') ", false, false);
    ...
    public function lead($id = '')
    {
        ...
        if ($lead == null && is_numeric($id)) {
            ...
            $data['members'] = $this->staff_model->getc('', 1, array(
                'is_not_staff' => 0
            ),false, true);
        ...
        $partner = $this->partner_model->get(get_staff_partner_id())[0];
        $data['cidade'] = $partner['cidade'] ?? '';
        $data['estado'] = $partner['estado'] ?? '';
    ...
    public function update_all_proposal_emails_linked_to_lead($id)
    {
        ...
        if ($this->input->post('update')) {
            ...
            $proposals     = $this->proposals_model->getc('', array(
                'rel_type' => 'lead',
                'rel_id' => $id
            ));
            $affected_rows = 0;
    ...
    public function convert_to_customer()
    {
        if ($this->input->post()) {
            ...
            if(!is_admin())
                $data['partner_id'] = get_staff_partner_id(get_staff_user_id());
    ...
    public function import()
    {
        ...
        $data['members'] = $this->staff_model->getc('', 1,array(),false,true);
