    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            ...
            if ($field['type'] == 'input') {
                ...
                $fields_html .= '<div class="form-group" style="padding-top: 1px;">';
                $fields_html .= '<label for="custom_fields[' . $field['fieldto'] . '][' . $field['id'] . ']">' . $field_name . '</label>';
                $fields_html .= '<select ' . $select_attrs . ' name="'.$select_name.'" class="selectpicker form-control" data-width="100%" data-none-selected-text="' . _l('dropdown_non_selected_tex') . '"  data-live-search="true" required>';
