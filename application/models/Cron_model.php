...
class Cron_model extends CRM_Model
{
    ...
    private function scheduled_tickets(){
    ...
    private function repair_tables(){
        ...
        foreach ($tables as $table){
            $check = $this->db->query("CHECK TABLE " . $table)->result_array();
            if($check[0]['Msg_text'] !== 'OK'){
                if ($this->dbutil->repair_table($table))
                {
                    logActivity('Tabela ' . $table . ' reparada com sucesso', null);
                }
            }
        }
    }
