...
class Clients_model extends CRM_Model
{
    ...
    public function get($id = '', $where = array('tblclients.active' => 1))
    {
        ...
        //$this->db->join('tblissuingcertificate', 'tblissuingcertificate.id = tblclients.issuing_certificate');
     ...
     * @return array
     */
    public function get_contacts($customer_id = '', $where = array('active' => 1), $concat = false)
    {
        $this->db->where($where);
        if ($customer_id != '') {
    ...
    public function check_cnpj($cnpj){
        $cliente = $this->db->where('cnpj_or_cpf', $cnpj)
            ->get('tblclients')->row();
        if($cliente){
            return true;
        }
    ...
    public function get_contact_related_clientcontact($id = '', $where = array()){
        $this->db->where($where);
        ...
        return $this->db
            ->select('tblcontacts.*, tblclientscontacts.id as clientcontactid, tblclientscontacts.clientid, tblclientscontacts.contactid')
            ->join('tblclientscontacts', 'tblclientscontacts.contactid = tblcontacts.id', 'left')
            ->get('tblcontacts')->row();
    }

    public function get_contacts_by_where($where = array()){
        $this->db->where($where);

        return $this->db
            ->get('tblclientscontacts')->result_array();
    }
    ...
    public function save_connect_contact($client_id, $contact_id){
        ...
        $check = $this->db
            ...
            ->get('tblclientscontacts');

        if($check->num_rows() <= 0){
            $id = $this->db->insert('tblclientscontacts', array(
                'clientid' => $client_id,
    ...
    public function get_clients_relation($contactid){
        return $this->db
                ->select('tblclientscontacts.id as id, tblclientscontacts.clientid as clientid, tblclientscontacts.contactid as contactid, tblclients.company as company')
                ->join('tblclients', 'tblclients.userid = tblclientscontacts.clientid')
...
//        }
        ...
        if (isset($data['is_primary'])) {
            $data['is_primary'] = 1;
//            $exist = $this->db->where('tblclientscontacts.clientid', $customer_id)->get('tblclientscontacts')->row();
//            if($exist) {
//                $this->db->where('tblclientscontacts.clientid', $customer_id);
//                $this->db->join('tblclientscontacts', 'tblclientscontacts.contactid = tblcontacts.id', 'left');
//                $this->db->update('tblcontacts', array(
//                    'is_primary' => 0
//                ));
//            }
     ...
     * @param bool $client_or_lead_convert_request
    ...
    public function add($data, $client_or_lead_convert_request = false)
    {
        ...
        if($data['validate_certificate'] != '') {
            $data['validate_certificate'] = \Carbon\Carbon::createFromFormat('d/m/Y', $data['validate_certificate'])
                ->format('Y-m-d');
        }else{
            $data['validate_certificate'] = null;
        }
        $data['datecreated'] = date('Y-m-d H:i:s');
        ...
        if($data['cnpj_or_cpf'] != ''){
            $data['cnpj_or_cpf'] = str_replace(array(".", "-", "/"), "", $data['cnpj_or_cpf']);
        }
        if($data['observation'] == '')
            $data['observation'] = null;
        else
            $data['observation'] = str_replace(array("\r\n", "\r", "\n"),"</br>",$data['observation']);
...
//            ));
    ...
    public function get_userid_by_cnpj($cnpj, $active = 1){
            $this->db->where('cnpj_or_cpf', $cnpj);
//            $this->db->where('active', $active);
            return $this->db->get('tblclients')->row();
    }

    public function get_userid_by_cod_empresa($cnpj, $cod_empresa, $active = 1){
//        $this->db->where('cnpj_or_cpf', $cnpj);
...
//            $this->db->where('active', $active);
    ...
    public function update($data, $id, $client_request = false)
    {
        if(!is_partner()) {
            ...
            if ($data['partner_id'] == '') {
                $data['partner_id'] = 0;//get_staff_partner_id(get_staff_user_id());
            }
            if ($data['cnpj_or_cpf'] != '') {
                $data['cnpj_or_cpf'] = str_replace(array(".", "-", "/"), "", $data['cnpj_or_cpf']);
            }
            ...
            trim($data['validate_certificate']);

            if (is_date($data['validate_certificate'])) {
...
//            dd($data);
            ...
            if ($data['observation'] == '')
                $data_p['observation'] = null;
            else
                $data['observation'] = str_replace(array("\r\n", "\r", "\n"),"</br>",$data['observation']);

            $this->db->where('userid', $id);
            $this->db->update('tblclients', $data_p);
            if ($this->db->affected_rows() > 0) {
                return true;
            }
            return false;
    ...
    public function delete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            // Delete all many-to-many relations
            $this->db->where('clientid', $id);
            $this->db->delete('tblclientscontacts');
    ...
    public function delete_relation_contact($clientcontact){

        $n = $this->db
            ->where('contactid', $clientcontact->contactid)
            ->get('tblclientscontacts')->result_array();

        if(count($n) > 1){
            $this->db->where('id', $clientcontact->id);
            $this->db->delete('tblclientscontacts');
            if($this->db->affected_rows() > 0){
                return true;
            }
        }else{
            $this->delete_contact($clientcontact->contactid);
        }
        return false;
    }
    ...
    public function change_client_status($id, $status)
    {
        ...
        // Faz com que caso um cliente seja desativado, todos os seus contatos também serão
//        $this->db->where('userid', $id);
//        $this->db->update('tblcontacts', array(
//            'active' => $status
//        ));
    ...
    public function change_client_atualiza($id, $status)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            logActivity('Status de atualização de cliente alterado [CustomerID: ' . $id . ' Status(Atualiza/Nao Atualiza): ' . $status . ']');
