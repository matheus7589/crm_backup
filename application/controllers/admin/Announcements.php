...
class Announcements extends Admin_controller
{
    ...
    public function index()
    {
        ...
        $data['staff'] = $this->db->query("SELECT staffid, CONCAT(firstname,' ',lastname) as name FROM tblstaff".$where)->result_array();

        if($this->input->post())
        {
            $dataa = $this->input->post();
            $all = $dataa['send_all'];
            $department = $dataa['send_department'];
            if($dataa['show_name'] == "on")
            {
                $dataa['fromuserid'] = get_staff_user_id();
                $dataa['from_fullname'] = get_staff_full_name();
            }
            else
            {
                $dataa['fromuserid'] = 0;
            ...
            unset($dataa['show_name']);
            if($all == "on")
            {
                unset($dataa['send_all']);
                $staffs = $data['staff'];
                unset($dataa['to']);
                foreach ($staffs as $staff)
                {
                    $dataa['touserid'] = $staff['staffid'];
                    $dataa['date'] = date('Y-m-d H:i:s');
                    $this->db->insert('tblnotifications',$dataa);
//                    refresh_notification($dataa['touserid']);
                }
            }
            else if($department == "on"){
                unset($dataa['send_department']);
                $staffs = $data['staff'];
                $to_dpt = $dataa['to'];
                unset($dataa['to']);
                foreach ($staffs as $staff)
                {
                    $dpt = $this->db->query("SELECT staffid, departmentid FROM tblstaffdepartments WHERE staffid=".$staff['staffid'])->result_array();
                    foreach ($dpt as $d){
                        if(in_array($to_dpt, $d)) {
                            $dataa['touserid'] = $staff['staffid'];
                            $dataa['date'] = date('Y-m-d H:i:s');
                            $this->db->insert('tblnotifications', $dataa);
                        }
            ...
            {
//                Adicionar quem mandou a mensagem e se é master.
                $dataa['touserid'] = $dataa['to'];
                $dataa['date'] = date('Y-m-d H:i:s');
                unset($dataa['to']);
                $this->db->insert('tblnotifications',$dataa);
                refresh_notification($dataa['touserid']);
    ...
    public function change_to_department(){
        $departments = $this->db->select('departmentid, name')
            ->get('tbldepartments')->result_array();

        $data = '<option></option>';
        foreach ($departments as $department){
            $data .= '<option value=' . $department["departmentid"] . '>' . $department["name"] . '</option>';
        }

        echo $data;
    }

    public function change_to_staff(){
        $staffs = $this->db->select('staffid, CONCAT(firstname, "", lastname) as name')
            ->get('tblstaff')->result_array();

        $data = '<option></option>';
        foreach ($staffs as $staff){
            $data .= '<option value=' . $staff["staffid"] . '>' . $staff["name"] . '</option>';
        }
        echo $data;
    }
