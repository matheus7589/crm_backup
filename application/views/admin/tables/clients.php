...
$aColumns = array(
    ...
    'email',
    'MAX(tblversoes.versao) as versao',
    'tblclients.phonenumber as number',
    'tblclients.active',
    '(SELECT GROUP_CONCAT(name ORDER BY name ASC) FROM tblcustomersgroups LEFT JOIN tblcustomergroups_in ON tblcustomergroups_in.groupid = tblcustomersgroups.id WHERE customer_id = tblclientscontacts.clientid) as groups',
    'tblclients.cnpj_or_cpf as cnpj',
    'tblclients.social_reason as social_reason',
...
array_push($join, ' LEFT JOIN tblclientscontacts ON tblclientscontacts.clientid = tblclients.userid ');

array_push($join, ' LEFT JOIN tblcontacts ON tblcontacts.id = tblclients.primary_contact ');

array_push($join, 'LEFT JOIN tblversoes ON tblclients.userid = tblversoes.userid ');
...
//    array_push($join, 'LEFT JOIN tblcustomfieldsvalues as ctable_' . $key . ' ON tblclientscontacts.clientid = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id']);
//}
...
if (count($groupIds) > 0) {
    array_push($filter, 'AND tblclientscontacts.clientid IN (SELECT customer_id FROM tblcustomergroups_in WHERE groupid IN (' . implode(', ', $groupIds) . '))');
}
...
if (count($invoiceStatusIds) > 0) {
    array_push($filter, 'AND tblclientscontacts.clientid IN (SELECT clientid FROM tblinvoices WHERE status IN (' . implode(', ', $invoiceStatusIds) . '))');
}
...
if (count($estimateStatusIds) > 0) {
    array_push($filter, 'AND tblclientscontacts.clientid IN (SELECT clientid FROM tblestimates WHERE status IN (' . implode(', ', $estimateStatusIds) . '))');
}
...
if (count($projectStatusIds) > 0) {
    array_push($filter, 'AND tblclientscontacts.clientid IN (SELECT clientid FROM tblprojects WHERE status IN (' . implode(', ', $projectStatusIds) . '))');
}
...
if (count($proposalStatusIds) > 0) {
    array_push($filter, 'AND tblclientscontacts.clientid IN (SELECT rel_id FROM tblproposals WHERE status IN (' . implode(', ', $proposalStatusIds) . ') AND rel_type="customer")');
}
...
if (count($contractTypesIds) > 0) {
    array_push($filter, 'AND tblclientscontacts.clientid IN (SELECT client FROM tblcontracts WHERE contract_type IN (' . implode(', ', $contractTypesIds) . '))');
}
...
if($this->_instance->input->post('active') != null){
    $partner = "";
    if(is_partner() || (isset($_SESSION['partner_id'])) && $_SESSION['partner_id'] != 0){
        $partner = " AND tblclients.partner_id = '" . get_staff_partner_id() . "' ";
    }
    if($this->_instance->input->post('active') == 1) {
        array_push($where, ' AND tblclients.active = 1 ' . $partner);
    }else {
        array_push($where, ' AND tblclients.active != 1 ' . $partner);
    }
}
...
if (count($customAdminIds) > 0) {
    array_push($filter, 'AND tblclientscontacts.clientid IN (SELECT customer_id FROM tblcustomeradmins WHERE staff_id IN (' . implode(', ', $customAdminIds) . '))');
}
...
if (!has_permission('customers', '', 'view')) {
    array_push($where, 'AND tblclientscontacts.clientid IN (SELECT customer_id FROM tblcustomeradmins WHERE staff_id=' . get_staff_user_id() . ')');
}
...
//array_push($where, ' AND is_primary = 1 ');

$result  = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, array(
//    'tblclients.cnpj_or_cpf as cnpj',
//    'tblclients.social_reason as social_reason',
//      'tblclients.primary_contact as primary_contact',
    ), ' GROUP BY userid ', 1);
...
foreach ($rResult as $aRow) {
    ...
    // versao
    $row[] = ($aRow['versao'] ? '<span class="label label-success">' . $aRow['versao'] . '</span>' : 'Não possui versão salva');

    // Primary contact phone
    $row[] = ($aRow['number'] ? '<a href="tel:' . $aRow['number'] . '">' . $aRow['number'] . '</a>' : '');
...
//    }
    ...
    $row[] = $aRow['social_reason'];

    //$row[] = $aRow['tblclients.active'];
    ...
    if ($hasPermissionDelete && !is_partner()) {
