...
class Authentication_model extends CRM_Model
{
    ...
    public function loginauto($email, $cnpj, $remember, $staff, $_data, $open_ticket = false) //na verdade email seria o $cpf
    {

        $this->load->model("staff_online_model");
        ...
        if (!empty($cnpj)) {
            ...
            if(empty($email)){
                $idcontato = $this->db
                    ...
                    ->where('cnpj_or_cpf', $cnpj)
                    ->get('tblclients')->result_array();

                $user = $this->db
                    ->where('id', $idcontato[0]['primary_contact'])
                    ->get($table)->row();
            }else{
                $this->db->join('tblclientscontacts', 'tblclientscontacts.contactid = tblcontacts.id');
                $this->db->join('tblclients', 'tblclients.userid = tblclientscontacts.clientid');
                $this->db->where('cpf', $email);
                $this->db->where('cnpj_or_cpf', $cnpj);
                $user = $this->db->get($table)->row();

//                if(!$user) {
//                    if ($data != '') {
//                        $this->db->where('firstname', $data['nome']);
...
//                }
            ...
            if ($user) {
                $user->id = $user->contactid ?? $user->id;
                // CPF is okey lets check the cnpj now
                $get = $this->db
                    ->join('tblclients', 'tblclients.userid = tblclientscontacts.clientid')
                    ->where('contactid', $user->id)
                    ->where('tblclients.cnpj_or_cpf', $cnpj)
                ...
                if($client){
                    //inserir novo contato aqui
                    $data = [];
                    $data['firstname'] = $_data['nome'] ?? 'Temporário';
                    $data['lastname'] = " ";
                    $data['email'] = '';
                    $data['cpf'] = $email;
                    ...
                    $id = $this->clients_model->add_contact($data, $client->userid, false);
                    ...
                    logActivity('Failed Login Attempt [User:' . $email . ', Is Staff Member:' . ($staff == true ? 'Yes' : 'No') . ', IP:' . $this->input->ip_address() . ']');

                    return false;
                }
            }
            if ($user->active == 0) {
                logActivity('Inactive User Tried to Login [User:' . $email . ', Is Staff Member:' . ($staff == true ? 'Yes' : 'No') . ', IP:' . $this->input->ip_address() . ']');
            ...
            if ($staff == true) {
                ...
                if($open_ticket)
                    $user_data['open_ticket'] = $open_ticket;
            ...
            if (!$twoFactorAuth) {
                if ($remember) {
                    $this->create_autologin($user->$_id, $staff, $open_ticket);
                }
                if ($staff) {
                    // registrar login no banco de dados
                    $this->staff_online_model->online($user->$_id);
                }
...
//            dd($staff);
            ...
            if ($staff == true) {
                ...
                if (!$twoFactorAuth) {
                    ...
                    $user_data = array(
                        'staff_user_id' => $user->$_id,
                        'staff_logged_in' => true,
                        'first_pending_notification' => true
                    );
                } else {
                    $user_data = array();
                    if ($remember) {
    ...
    public function logout($staff = true)
    {
        ...
        if (is_client_logged_in()) {
            ...
            $this->session->unset_userdata('first_pending_notification');
        } else {

            $this->load->model("staff_online_model");
            $this->staff_online_model->offline(get_staff_user_id());
     ...
     * @return boolean
     */
    private function create_autologin($user_id, $staff, $open_ticket = false)
    {
