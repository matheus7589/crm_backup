...
{
    $ativação = array(
        'name'=>'activation_quantum',
        'url'=>admin_url('clients/client/'.$client->userid.'?group=activation_quantum'),
        'icon'=>'fa fa-key',
        'lang'=>'Ativação',
        'visible'=>QuantumPrivateSenha::GetPermissionPass(),
        'order'=>17
    );
}
...
{
    $ativação = array(
        'name'=>'activation_inorte',
        'url'=>admin_url('clients/client/'.$client->userid.'?group=activation_inorte'),
        'icon'=>'fa fa-key',
        'lang'=>'Ativação',
        'visible'=>((get_staff_user_id() == 44) and (PAINEL == INORTE)) ? true : false,
        'order'=>17
    );
}
    array_push($customer_tabs,$ativação);
